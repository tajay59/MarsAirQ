      ...
      <div class=" w-full h-dvh flex flex-col" >
        ...
        <div class=" h-20 flex flex-col justify-center ml-16 " >
            <VBtnGroup  variant="outlined" divided density="compact" rounded="lg" class=""   >
                <VBtn v-for="comp in compToRender.list" :text="comp.text"    :active="(compToRender.selected == comp.name)"    :class="[(compToRender.selected == comp.name)?   'font-bold': 'font-regular']"   color="onSurface"   class="text-caption " @click="compToRender.selected = comp.name"   size="x-small" ></VBtn>
            </VBtnGroup>  

            <Transition name="slide-right"  >                      
                <VCard  rounded="0"  v-show="compToRender.selected == 'accounts'" flat  height="100%" width="270" color="surface"   class=" absolute top-0 left-[-270px] z-[22000]" >
                    <VCardItem class="mt-2 mb-10"  >
