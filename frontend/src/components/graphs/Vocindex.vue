...
<script setup>
   ...
   import  'highcharts/highcharts-more';
    ...
    watch(payload,(msg)=> {          
        ...
        if(msg.type == "station" && params.value.includes('vocindex')  && msg.id == selectedStation.value){              
            chart.value.setTitle({text: _.toUpper(msg.name)});
            chart1.value.setTitle({text: _.toUpper(msg.name)})
            chart.value.series[0].addPoint({y:parseFloat(msg.data.vocindex.toFixed(2)) ,x: msg.timestamp * 1000 }, true, shift.value);   
        }          
    });
    ...
    chart.value = Highcharts.chart('container', {
        ...
        subtitle: { text: 'Vocindex', align: 'center' },
        yAxis: { 
            title:null,// title: { text: 'Vocindex' , style:{color:'#000000'}},
            labels: { format: '{value}  ' },   
            // max:50,
            // min:10        
        },
