...
<script setup>
   ...
   import  'highcharts/highcharts-more';
    ...
    watch(payload,(msg)=> {          
        ...
        if(msg.type == "station" && params.value.includes('uvc')){    
            liveData.value.uvc = msg.data.uvc          
            chart.value.setTitle({text: _.toUpper(msg.name)});
            chart1.value.setTitle({text: _.toUpper(msg.name)});
            chart.value.series[0].addPoint({y:parseFloat(msg.data.uvc.toFixed(2)) ,x: msg.timestamp * 1000 }, true, shift.value);   
        }          
    });
    ...
    chart.value = Highcharts.chart('container', {
        ...
        subtitle: { text: 'UVC', align: 'center' },
        yAxis: { 
            title:null,// title: { text: 'UVC' , style:{color:'#000000'}},
