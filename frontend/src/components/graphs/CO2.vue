...
<script setup>
   ...
   import  'highcharts/highcharts-more';
    ...
    watch(payload,(msg)=> {          
        ...
        if(msg.type == "station" && params.value.includes('co2')){    
            liveData.value.co2 = msg.data.co2          
            chart.value.setTitle({text: _.toUpper(msg.name)});
            chart1.value.setTitle({text: _.toUpper(msg.name)});
            chart.value.series[0].addPoint({y:parseFloat(msg.data.co2.toFixed(2)) ,x: msg.timestamp * 1000 }, true, shift.value);   
        }          
    });
    ...
    chart.value = Highcharts.chart('container', {
        ...
        subtitle: { text: 'CO2', align: 'center' },
        yAxis: { 
            title:null,// title: { text: 'CO2' , style:{color:'#000000'}},
