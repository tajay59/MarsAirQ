...
<script setup>
   ...
   import  'highcharts/highcharts-more';
    ...
    watch(payload,(msg)=> {          
        ...
        if(msg.type == "station" && params.value.includes('uvb')){    
            liveData.value.uvb = msg.data.uvb          
            chart.value.setTitle({text: _.toUpper(msg.name)});
            chart1.value.setTitle({text: _.toUpper(msg.name)});
            chart.value.series[0].addPoint({y:parseFloat(msg.data.uvb.toFixed(2)) ,x: msg.timestamp * 1000 }, true, shift.value);   
        }          
    });
    ...
    chart.value = Highcharts.chart('container', {
        ...
        subtitle: { text: 'UVB', align: 'center' },
        yAxis: { 
            title:null,// title: { text: 'UVB' , style:{color:'#000000'}},
