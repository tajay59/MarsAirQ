...
<script setup>
   ...
   import  'highcharts/highcharts-more';
    ...
    watch(payload,(msg)=> {          
        ...
        if(msg.type == "station" && params.value.includes('uva')){    
            liveData.value.uva = msg.data.uva          
            chart.value.setTitle({text: _.toUpper(msg.name)});
            chart1.value.setTitle({text: _.toUpper(msg.name)});
            chart.value.series[0].addPoint({y:parseFloat(msg.data.uva.toFixed(2)) ,x: msg.timestamp * 1000 }, true, shift.value);   
        }          
    });
    ...
    chart.value = Highcharts.chart('container', {
        ...
        subtitle: { text: 'UVA', align: 'center' },
        yAxis: { 
            title:null,// title: { text: 'UVA' , style:{color:'#000000'}},
