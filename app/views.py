    ...
    if request.method == "POST":       
            ...
            try:
                ...
                if  unhash and account:
                    message = {"status":"loggedIn","message":"loggedIn","username":account['lastname'],"email":account['email'],"id":account['id'],"role":account['role'],"suball": account["suball"],"image":account["image"],"web": account["web"]}
                    account.pop("email") 
                    account.pop("password")  
                    account.pop("timestamp") 
                    account.pop("registered") 
    ...
    if request.method == 'POST':
        ...
        if doc:
            ...
            if updateDoc: 
                ...
                msg = Message("Verification code",sender="AQ.donotreply@cimh.edu.bb" , recipients=[email])                        
            ...
            espireAt    = datetime.now(timezone(timedelta(hours= timeOffset))) + timedelta(minutes=5)
            query       = { "expireAt":espireAt,"type":"registration","firstname":firstname,"lastname":lastname,"email":email,"organization": organization,"country": country,"code":code,"password":password,"image":"default.png" }
        ...
        if doc: 
            ...
            if doc['code'] == code and doc['email'] == email:
                ...
                if "@cimh.edu.bb" in email:
                    ...
                    user = {"id":f"ACC{token_hex(16)}","firstname":firstname,"lastname":lastname,"email":email,"organization": organization,"country": country,"password": pwdHash,"role":"staff","timestamp":timestamp,"registered":registered,"emailconfirmed":True,"activated": True,"suball": False,"image":"default.png" }
                    
                else:
                    # REGISTRATION FROM EXTERNAL USER
                    user = {"id":f"ACC{token_hex(16)}","firstname":firstname,"lastname":lastname,"email":email,"organization": organization,"country": country,"password": pwdHash,"role":"user","timestamp":timestamp,"registered":registered,"emailconfirmed":True,"activated": False,"suball": False,"image":"default.png"}
                ...
                if doc:
                    ...
                    if len(emailList) > 0:
                        # SEND EMAIL TO USER WITH VERIFICATION CODE 
                        msg = Message("AQ - New Account Alert",sender="AQ.donotreply@cimh.edu.bb" , recipients=[x['email'] for x in  emailList])                      
                    ...
                    msg = Message(f"Mars AirQ Account Request",sender="AQ.donotreply@cimh.edu.bb" , recipients=[email])                        
                    msg.html    = f'''<!DOCTYPE html> <html> <head> <title>Mars AirQ</title> </head> <body>
                        <div><p>  Dear Customer, </p></div>
                        <div><p>  Thank you for your interest in a Mars AirQ account at the Caribbean Institute of Meteorology and Hydrology. Your information will be processed and a follow up email sent there after.</p></div>
...
def approveOrDeny():   
    if request.method == "POST": 
        ...
        if account["role"] == "admin":
            ...
            if doc:
                # process reequest Approve or Deny
                ...
                if decision == "approve":
                    ...
                    web               = {} 
                    web["id"]         = token_hex(16)
                    web["username"]   = token_hex(8)
                    web["password"]   = hash
                    web["type"]       = "web"
                    web["superuser"]  = False
                    web["owner"]      = newAccount["id"]
                    ...
                    exist1, notice       = mongo.mqttUserExist(id = web["id"])
                    ...
                    if exist and exist1 :
                        ...
                        emailList = emails['registration']
                        # SEND EMAIL TO USER WITH VERIFICATION CODE 
                        msg = Message("AQ - Account(Approved) Alert",sender="AQ.donotreply@cimh.edu.bb" , recipients=[x['email'] for x in  emailList])                      
                        msg.html    = f'A new Mars AirQ account was approved for {newAccount['firstname'].capitalize()} {newAccount['lastname'].capitalize()} with email: {newAccount['email']}. Login and visit the Admin section to see the account details.'
                        mail.send(msg) 

                        # SEND EMAIL TO USER ACKNOWLEDGING RECEIPT OF NEW ACCOUNT REQUEST 
                        msg = Message(f"Mars AirQ Account",sender="AQ.donotreply@cimh.edu.bb" , recipients=[newAccount['email']])                        
                        msg.html    = f'''<!DOCTYPE html> <html> <head> <title>Mars AirQ</title> </head> <body>
                            <div><p>  Dear Customer, </p></div>
                            <div><p>  We are pleased to inform you that your request for a Mars AirQ account at the Caribbean Institute of Meteorology and Hydrology has been approved. Please visit our website and log in to begin using the platform.</p></div>
                ...
                if decision == "deny":
                    ...
                    if count > 0:
                        ...
                        emailList = emails['registration']
                        # SEND EMAIL TO USER WITH VERIFICATION CODE 
                        msg = Message("AQ - Account(Denied) Alert",sender="AQ.donotreply@cimh.edu.bb" , recipients=[x['email'] for x in  emailList])                      
                        ...
                        msg.html    = f'''<!DOCTYPE html> <html> <head> <title>Mars AirQ</title> </head> <body>
                            <div><p>  Dear Customer, </p></div>
                            <div><p>  We regret to inform you that your request for a Mars AirQ account at the Caribbean Institute of Meteorology and Hydrology has not been approved.</p></div>
...
def getPage():   
    if request.method == "POST": 
        ...
        if account:
            if ops == "search":   
                data = mongo.getSearchPagination(searchtext= search,page= page, owner= account["id"], role= account["role"])   
            else:
